...
//detalles del participantes
      ...
      public function buscador_detalle_participantes($data){
        ...
        //$objeto = self::listado_imagenes(); 
          ...
          $this->db->select("AES_DECRYPT(p.ciudad,'{$this->key_hash}') AS ciudad", FALSE);  
          $this->db->select("AES_DECRYPT(p.contrasena,'{$this->key_hash}') AS contrasena", FALSE);
           $this->db->select("( CASE WHEN r.compra = 0 THEN '' ELSE DATE_FORMAT(FROM_UNIXTIME(r.compra),'%d-%m-%Y') END ) AS fecha_pc_compra", FALSE); 
          $this->db->select("(p.redes=1)* 100 AS TOTAL_PUNTOS_FACEBOOK", FALSE);    
          $this->db->select("COUNT(r.id_participante) as 'TICKETS_REGISTRADOS'");
          $this->db->select("sum(AES_DECRYPT(r.monto,'{$this->key_hash}') ) AS PUNTOS_OBTENIDOS_COMPRA", FALSE);
          ...
          //$this->db->select("r.responder AS responder", FALSE);      
          $this->db->select("( CASE WHEN r.responder = 1 THEN 'Si' ELSE 'No' END ) AS responder", FALSE);
